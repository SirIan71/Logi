...
export default function Routes() {
  const { routes, trips, income, expenses, lookup, addItem, updateItem, deleteItem, user } = useApp();
  ...
  const csvColumns = [
    ...
    ...(!isOps ? [{ label: 'Profit', accessor: r => r.profit }] : []),
  ];
    ...
    <div>
      ...
      <div className="table-container">
        ...
        <div style={{overflowX:'auto'}}>
          <table className="data-table">
            <thead><tr><th>Route</th><th>Distance</th><th>Est. Fuel</th>{!isOps && <th>Est. Tolls</th>}<th>Duration</th><th>Trips</th>{!isOps && <><th>Total Profit</th><th>Avg Profit/Trip</th></>}<th>Actions</th></tr></thead>
            <tbody>{filtered.map(r => (
              <tr key={r.id}>
                ...
                {!isOps && <td className="numeric">{formatCurrency(r.estimated_tolls)}</td>}
      ...
      {modal && <Modal title={modal==='add'?'Add Route':'Edit Route'} onClose={closeModal}
        ...
        <div className="form-grid">
          ...
          {!isOps && <div className="form-group"><label className="form-label">Est. Tolls (R)</label><input className="form-input" type="number" value={form.estimated_tolls||''} onChange={e=>setForm({...form,estimated_tolls:e.target.value})}/></div>}
