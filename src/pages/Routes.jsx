...
export default function Routes() {
  ...
  // Drivers only see routes used in their own trips
  const visibleRoutes = useMemo(() => {
    if (isOwnOnly) {
      const myRouteIds = new Set(trips.filter(t => t.driver_id === user?.id).map(t => t.route_id));
      return routes.filter(r => myRouteIds.has(r.id));
    }
    return routes;
  }, [routes, trips, isOwnOnly, user]);

  // Hide financial data from operations and drivers
  const showFinancials = canEdit && !isOwnOnly;

  const routeStats = useMemo(() => visibleRoutes.map(r => {
  ...
  const csvColumns = [
    ...
    { label: 'Distance', accessor: r => r.distance_km },
    { label: 'Trips', accessor: r => r.tripCount },
    ...(showFinancials ? [{ label: 'Profit', accessor: r => r.profit }] : []),
  ];
      ...
      <div className="table-container">
        ...
        <div style={{overflowX:'auto'}}>
          <table className="data-table">
            <thead><tr><th>Route</th><th>Distance</th><th>Est. Fuel</th>{showFinancials && <th>Est. Tolls</th>}<th>Duration</th><th>Trips</th>{showFinancials && <><th>Total Profit</th><th>Avg Profit/Trip</th></>}<th>Actions</th></tr></thead>
            <tbody>{filtered.map(r => (
              <tr key={r.id}>
                ...
                {showFinancials && <td className="numeric">{formatCurrency(r.estimated_tolls)}</td>}
